...
% Retrieve and round the frame rate to the nearest integer
frameRate = round(videoObj.FrameRate);
fprintf('Frame Rate (rounded): %d fps\n', frameRate);
...
% Calculate the maximum number of frames for 6 minutes
maxFrames = frameRate * 6 * 60; % 6 minutes * 60 seconds * frameRate
...
for i = 1:length(csv_files)
    ...
    % Limit to first maxFrames rows
    if height(csvData) > maxFrames
        csvData = csvData(1:maxFrames, :);
    end
...
adjustedHeatmapMax = heatmapMax * 0.20;  % Set the heatmap display max to 20% of the computed max
...
for i = 1:length(csv_files)
    ...
    timeInCentralArea = sum(inCentralArea) / frameRate;
    totalTime = length(MainBodyX) / frameRate;
