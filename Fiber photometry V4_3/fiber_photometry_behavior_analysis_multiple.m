% fiber_photometry_behavior_analysis_combined_sorted.m
...
%   5. Generates individual overlay plots with mean and SEM.
%   6. Creates individual heatmaps of the normalized signals.
% After processing all file pairs, it combines all Behavior1 and Behavior2 data
% into aggregate overlay plots and sorted heatmaps based on AUC from 0 to 100s post-tag.

function fiber_photometry_behavior_analysis_combined_sorted()
    ...
    %% 3. Process Each CSV and Excel File Pair Individually
    ...
    for i = 1:numCsvFiles
        ...
        %% 5. Apply Low-Pass Butterworth Filter to Smooth the Signal
        ...
        figure('Name', sprintf('Raw vs. Smoothed Real_Signal - %s', csvFileName), 'NumberTitle', 'off');
        ...
        title(sprintf('Raw vs. Smoothed Real\_Signal - %s', csvFileName));
        legend('show');
        grid on;
        hold off;

        %% 6. Load and Extract Behavior Tags from Excel File
        ...
        %% 8. Analyze and Visualize Behavior1 Tags
        ...
        %% 9. Analyze and Visualize Behavior2 Tags
    ...
    %% 10. Generate Aggregate Figures for All Behavior1 and Behavior2 Data

    % Note: This section is outside the main loop to ensure it runs after all file pairs are processed.

    % Check if aggregated data exists
    if exist('all_normalizedData1', 'var') && ~isempty(all_normalizedData1)
    ...
    if exist('all_normalizedData2', 'var') && ~isempty(all_normalizedData2)
